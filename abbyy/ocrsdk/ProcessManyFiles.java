...
public class ProcessManyFiles {
	...
	static String appId = "orzo-abbyy";
	static String password = "ezvhxJ4+eC5ppWIsBUKVL/B0";
	private static long startTime;
	public static void main(String[] args) {

		System.out.println(
				"Process multiple documents using ABBYY Cloud OCR SDK.\n");
		...
		try {
			if (mode.equalsIgnoreCase("help")) {
				...
				System.out.println("Estimated Total Running time "
						+ Math.floor(
	...
	private static void displayHelp() {
		System.out.println("Recognize multiple files at once.\n" + "Usage:\n"
	...
	private static void displayRecognizeHelp() {
		System.out.println("Recognize all images from directory.\n" + "\n"
				+ "Usage:\n"
	...
	private static void performRecognition(Vector<String> argList)
		...
		settings.setLanguage(
				CmdLineOptions.extractRecognitionLanguage(argList));
	...
	private static void displayRemoteHelp() {
		displayBetaWarning();
		System.out.println("Recognize images specified by URL.\n" + "\n"
				+ "Usage:\n"
	 ...
	 */
	private static Map<String, String> submitAllFiles(Vector<String> fileList,
			ProcessingSettings settings) throws Exception {
		System.out.println(
				String.format("Uploading %d files..", fileList.size()));
	...
	private static Map<String, String> submitRemoteUrls(Vector<String> urlList,
			ProcessingSettings settings) throws Exception {
		System.out.println(
				String.format("Processing %d urls...", urlList.size()));
		Map<String, String> taskIds = new HashMap<String, String>();
	 ...
	 */
	private static void waitAndDownloadResults(Map<String, String> taskIds)
			throws Exception {
		// Call listFinishedTasks while there are any not completed tasks from
		// taskIds

		// Please note: API call 'listFinishedTasks' returns maximum 100 tasks
		// So, to get all our tasks we need to delete tasks on server. Avoid
		// running
		// parallel programs that are performing recognition with the same
		// Application ID
			...
			for (int i = 0; i < finishedTasks.length; i++) {
				...
				if (taskIds.containsKey(task.Id)) {
					...
					if (task.Status == Task.TaskStatus.Completed) {
						...
						restClient.downloadResult(task, task.DownloadUrl,
								outputPath);

						Integer lastIndexOfDot = outputPath.lastIndexOf(".");
						String outputXmlFilePath = outputPath.substring(0,
								lastIndexOfDot) + ".xml";
						restClient.downloadResult(task, task.DownloadUrl1,
								outputXmlFilePath);

						System.out.println(String.format("Ready %s, %d remains",
								fileName, taskIds.size()));
					} else {
						System.out
								.println(String.format("Failed %s, %d remains",
										fileName, taskIds.size()));
					}

				} else {
					System.out.println(String
							.format("Deleting task %s from server", task.Id));
				}
