...
public class Client {
	...
	public Task processImage(String filePath, ProcessingSettings settings)
			throws Exception {
		URL url = new URL(
				serverUrl + "/processImage?" + settings.asUrlParams());
		System.out.println(settings.asUrlParams());
		return postFileToUrl(filePath, url);
	}
	...
	public Task processRemoteImage(String fileUrl, ProcessingSettings settings)
		...
		URL url = new URL(String.format("%s/processRemoteImage?source=%s&%s",
				serverUrl, URLEncoder.encode(fileUrl, "UTF-8"),
	...
	public Task processBusinessCard(String filePath, BusCardSettings settings)
			throws Exception {
		URL url = new URL(
				serverUrl + "/processBusinessCard?" + settings.asUrlParams());
		return postFileToUrl(filePath, url);
	}
	...
	public Task processReceipt(String filePath, ReceiptSettings settings)
			throws Exception {
		URL url = new URL(
				serverUrl + "/processReceipt?" + settings.asUrlParams());
		return postFileToUrl(filePath, url);
	}
	...
	public Task processTextField(String filePath, TextFieldSettings settings)
			throws Exception {
		URL url = new URL(
				serverUrl + "/processTextField?" + settings.asUrlParams());
		return postFileToUrl(filePath, url);
	}
	...
	public Task processBarcodeField(String filePath, BarcodeSettings settings)
			throws Exception {
		URL url = new URL(
				serverUrl + "/processBarcodeField?" + settings.asUrlParams());
		return postFileToUrl(filePath, url);
	}
	...
	/**
	 * Process and parse Machine-Readable Zone (MRZ) of Passport, ID card, Visa
	 * etc
	 * 
	 * For details see http://ocrsdk.com/documentation/apireference/processMRZ/
	 * 
	 */
	...
	public void downloadResult(Task task, String downloadUrl, String outputPath)
			throws Exception {
		...
		parseDownloadUrl(downloadUrl, outputPath);
		// parseDownloadUrl(task.DownloadUrl1, outputFile+"1");

	}

	private void parseDownloadUrl(String downloadUrl, String outputFile)
			throws IOException {
		System.out.print("Storing into: " + outputFile);
	...
	private byte[] readDataFromFile(String filePath) throws Exception {
		...
		try {
			...
			while (true) {
				...
				numRead = inputStream.read(dataBuffer, offset,
						dataBuffer.length - offset);
				if (numRead < 0) {
					break;
				}
				offset += numRead;
